...
public class IslandServiceImpl implements IslandService {
	...
	private UserDiaryDao userDiaryDao = BasicFactory.getFactory()
			.getDao(UserDiaryDao.class);
	private RemarkDiaryDao remarkDiaryDao = BasicFactory.getFactory()
			.getDao(RemarkDiaryDao.class);
	Gson gson = new Gson();
	...
	public String getHotIsland(String id, String password, String start,
		...
		for (Island island : islands) {
			UserIsland userIsland = userIslandDao.checkExistByUserAndIslandId(id, String.valueOf(island.getId()));
			if (null != userIsland) {
			...
			List<UserIsland> islandUsers = userIslandDao.getUserByIslandId(String.valueOf(island.getId()));
			List<IslandDiary> islandDiaryList = islandDiaryDao.getIslandDiarysByIslandId(String.valueOf(island.getId()));
...
//				if (!island2.getName().equals(island.getName())) {
					...
					UserIsland userIsland = userIslandDao.checkExistByUserAndIslandId(id, String.valueOf(island2.getId()));
					if (null != userIsland) {
						Map<String,String> mapTemp = new HashMap<String, String>();
						mapTemp.put("belong", "1");
						belongs.add(mapTemp);
					...
					islands.add(island2);
					List<UserIsland> islandUsers = userIslandDao.getUserByIslandId(String.valueOf(island2.getId()));
					List<IslandDiary> islandDiaryList = islandDiaryDao.getIslandDiarysByIslandId(String.valueOf(island2.getId()));
					Map<String, String> mapUserNum = new HashMap<String, String>();
					mapUserNum.put("userNum",String.valueOf(islandUsers.size()));
					mapUserNum.put("diaryNum",String.valueOf(islandDiaryList.size()));
					isalndInfoList.add(mapUserNum);
//				}
	...
	@Override
	public String getMengdaoPageInfo(String id, String password) {
		// TODO Auto-generated method stub
		Map<String, Object> map = new HashMap<String, Object>();
		// 1先检查用户
		UserInfo userInfo = userDao.checkUserById(id, password);
		...
		// 处理返回的信息
		recommendIslands = islandDao.getHotIsland("0", "10");
		if (null != recommendIslands && recommendIslands.size() > 0) {
			for (Island islandTemp : recommendIslands) {
				Map<String, String> mapTemp = new HashMap<String, String>();
				UserIsland userIslandExist = userIslandDao.checkExistByUserAndIslandId(id, String.valueOf(islandTemp.getId()));
...
//				.getHotIslandDiary(0, 10);
		List<IslandDiary> hotIslandDiaryList = getHotIslandDiary();
		...
		if (null != hotIslandDiaryList && hotIslandDiaryList.size() > 0) {
			for (IslandDiary islandDiary : hotIslandDiaryList) {
				...
				UserDiary userDiary = userDiaryDao.getDiaryBelongs(String.valueOf(diaryTemp.getId()));
				if (null != userDiary) {
					UserInfo userInfo2 = userDao.findUserById(String.valueOf(userDiary.getUser_id()));
					...
					if (null != diaryTemp&& null != userInfo2) {
						...
						mapTemp.put("userName", String.valueOf(userInfo2.getName()));
	...
	private List<IslandDiary> getHotIslandDiary() {
		...
		for (int i = 0; i < todayDiaryTemp.size(); i++) {
			...
			List<IslandDiary> islandDiaries = islandDiaryDao.findDiaryByDiaryId(String.valueOf(diary.getId()));
		...
		//3 获取虽有这些随记的评论
		if (null != todayIslandDiaryListTemp && todayIslandDiaryListTemp.size() > 0) {
			for (int i = 0; i < todayIslandDiaryListTemp.size(); i++) {
				IslandDiary islandDiary = todayIslandDiaryListTemp.get(i);
				List<RemarkDiary> remarkList = remarkDiaryDao.getItemsByDiaryId(String.valueOf(islandDiary.getDiary_id()));
				if (remarkList != null) {
		...
		for (int i = 0; i < todayIslandDiaryListTemp.size()-1; i++) {
			for (int j = i; j < todayIslandDiaryListTemp.size()-1; j++) {
				...
				if (num < remarNum.get(j+1)) {
					todayIslandDiaryListTemp.set(j, todayIslandDiaryListTemp.get(j+1));
	...
	public String attentionIsland(String id, String password, String islandId) {
		...
		UserIsland userIslandExist = userIslandDao.checkExistByUserAndIslandId(id, islandId);
		if (null != userIslandExist) {
			map.put("flag", "false");
	...
	public String checkDiary2Island(String id, String password,
		...
		IslandDiary islandDiary = islandDiaryDao.getItemByDiaryIdAndIslangId(diaryId,islandId);
		if (null == islandDiary) {
			map.put("flag", "false");
