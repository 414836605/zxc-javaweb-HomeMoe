...
public class DiaryServiceImpl implements DiaryService {
	...
	@Override
	public String getGroupDiary(String id, String password, String groupId) {
		...
		getUserInfoAndDiaryInfo(id, diaryListTemp, diaryList, userList);
		orderByRemarkDiary(diaryList, userList);
		// ---------------
	...
	private void orderByRemarkDiary(List<Diary> diaryList,
		...
		for (int i = 0; i < diaryList.size(); i++) {
			List<RemarkDiary> remarkDiaryTemp = remarkDiaryDao
					.getItemsByDiaryId(String.valueOf(diaryList.get(i).getId()));
	...
	@Override
	public String getDiaryRemark(String id, String password, String diaryId) {
		Map<String, String> map = new HashMap<String, String>();
		// 1 验证用户密码是否正确
		UserInfo userInfo = userDao.checkUserById(id, password);
		if (null == userInfo) {
		...
		UserDiary diaryBelongs = userDiaryDao.getDiaryBelongs(diaryId);
		// //如果是自己的则全部显示 不是自己的显示好友的
		// if (diaryBelongs.getUser_id() == Integer.parseInt(id)) {
		//
		// }

		List<Diary> diaryList = new ArrayList<Diary>();
		List<Diary> diaryListTemp = getDiaryRemarkList(diaryId);
	 ...
	 * @param diaryId
	 */
	private List<Diary> getDiaryRemarkList(String diaryId) {
		// TODO Auto-generated method stub
		List<RemarkDiary> remarkDiaryList = remarkDiaryDao
				.getItemsByDiaryId(diaryId);
	...
	public String getFriendCircleDiary(String id, String password,
		...
		// // 2找出所有的好友
		// List<UserRelation> friendList = userRelationDao.getAllFriends(id);
		// // if (friendList.size() <= 0) {// 没有好友则显示自己的记录
		// // friendList.add(id)
		// // // return getMyShareToFriendDiary(id, password, start, offset);
		// // }
		//
		// String friends = "";
		// for (int i = 0; i < friendList.size(); i++) {
		//
		// if (id.equals(String.valueOf(friendList.get(i).getUser_id1()))) {
		// friends += String.valueOf(friendList.get(i).getUser_id2())
		// + ",";
		// } else if (id.equals(String
		// .valueOf(friendList.get(i).getUser_id2()))) {
		// friends += String.valueOf(friendList.get(i).getUser_id1())
		// + ",";
		// }
		//
		// }

		String friends = getFriendIds(id);
		 friends += ","+ id + "";
		// 3 找出这些好友的随记
	...
	public List<Map<String, Object>> getReportsInfo(int page) {
		...
		for (UserDiary ud : uds) {
			Map<String, Object> reportInfo = new HashMap<String, Object>();
				...
				if (diaryDao.findDiaryById(ud.getId() + "") != null) {
					reportInfo.put("content",
							diaryDao.findDiaryById(ud.getId() + "")
									.getContent());
				} else {
					continue;
				}
				...
				reportInfo
						.put("publishPersion",
								userDao.getUserInfoById(
										userDiaryDao.getPersionByDiaryId(
												ud.getDiary_id()).getUser_id())
										.getId());
	...
	public String getLastFriendDiary(String id, String password) {
		...
		// 2获取所有的朋友
		String ids = getFriendIds(id);
		// // 2获取所有的朋友
		// List<UserRelation> allFriends = userRelationDao.getAllFriends(id);
		// String ids = "";
		// if (null != allFriends) {
		// for (int i = 0; i < allFriends.size(); i++) {
		//
		// if (i == allFriends.size() - 1) {
		// if (id.equals(String.valueOf(allFriends.get(i)
		// .getUser_id1()))) {
		// ids += allFriends.get(i).getUser_id2();
		// } else {
		// ids += allFriends.get(i).getUser_id1();
		// }
		//
		// } else {
		// if (id.equals(String.valueOf(allFriends.get(i)
	...
	public String getFriendIds(String id) {
		// 亲友
		List<UserRelation> allFriends = userRelationDao.getAllFriends(id);
		String ids = "";
		if (null != allFriends) {
		...
		// 群聊好友
		List<UserGroup> userGroups = userGroupDao.getGroupsByUserId(id);
		if (null != userGroups && userGroups.size() > 0) {
			...
			for (int i = 0; i < userGroups.size(); i++) {
				...
				List<UserGroup> groupMembers = userGroupDao
						.getGroupsByGroupId(userGroup.getGroupchat_id());
				if (null != groupMembers && groupMembers.size() > 0) {
					for (int j = 0; j < groupMembers.size(); j++) {
						UserGroup userGroup2 = groupMembers.get(j);
						if (i == userGroups.size() - 1
								&& j == groupMembers.size() - 1) {
							if (!ids.contains(String.valueOf(userGroup2
									.getUser_id()))) {

								ids += userGroup2.getUser_id();
							}
