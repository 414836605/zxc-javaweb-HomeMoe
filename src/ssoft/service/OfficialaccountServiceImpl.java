...
public class OfficialaccountServiceImpl implements OfficialaccountService {
	...
	public String followOfficial(String id, String password, String officialId,
		...
		if (type.equals("0")) {
			...
			result = userOfficialaccountDao.addUser(
					Integer.parseInt(officialId), id, "1");
			if (result < 0) {
				map.put("flag", "false");
				map.put("errorCode", "-2");// -2 数据库错误
				return gson.toJson(map);
	...
	public List<Map<String, Object>> getCommunitiesInfo(int page) {
		...
		for (Officialaccounts community : communities) {
			...
			communityInfo.put("createPeople", createPeople);
			...
			communityInfo.put("postNum", officialaccountDiaryDao
					.getOfficialDiarysByOfficialId(community.getId() + "").size());
	...
	public List<Map<String, Object>> getVillagesInfo(int page) {
		...
		for (Officialaccounts village : villages) {
			...
			villageInfo.put("createPeople", createPeople);
			villageInfo.put("createTime", village.getCreatetime());
			villageInfo.put("attentionNum", userOfficialaccountDao
					.getOfficialaccountsByOfficialaccountId(village.getId())
					.size());
			villageInfo.put("postNum", officialaccountDiaryDao
					.getOfficialDiarysByOfficialId(village.getId() + "").size());
	...
	public List<Map<String, Object>> getSchoolsInfo(int page) {
		...
		for (Officialaccounts school : schools) {
			...
			schoolInfo.put("createPeople", createPeople);
...
//					Map<String, Object> tempmap = new HashMap<>();
					...
					// json转成list
					corrdinateList = gson.fromJson(corrdinate, ArrayList.class);
					for (Object json : corrdinateList) {
						...
						double distance = Math.acos(Math.sin(myLatitude)
								* Math.sin(tempLatitude) + Math.cos(myLatitude)
						...
						if (distance < 100) {
							...
							List<UserOfficialaccount> userOfficialaccounts= userOfficialaccountDao.getOfficialaccountsByOfficialaccountId(officialaccount.getId());
							if (null != userOfficialaccounts && userOfficialaccounts.size() > 0) {
								tempmap.put("memberNum", String.valueOf(userOfficialaccounts.size()));
							}else{
	...
	public String checkDiary2Official(String id, String password,
		...
		OfficialaccountDiary officialaccountDiary = officialaccountDiaryDao.getItemByDiaryIdAndOfficalId(diaryId,officialId);
